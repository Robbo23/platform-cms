...
export async function getPost(req, res, session) {
...
export async function deletePost(req, res) {
  ...
  if (Array.isArray(postId))
    return res.status(400).end('Bad request. postId parameter cannot be an array.');
  ...
  try {
    ...
    if (response) {
      await revalidate(
        `https://${response.site?.subdomain}.platform-cms.vercel.app`,
        response.slug
...
export async function updatePost(req, res) {
  ...
  try {
    ...
    if (subdomain) await revalidate(`https://${subdomain}.platform-cms.vercel.app`, slug); // revalidate for subdomain
